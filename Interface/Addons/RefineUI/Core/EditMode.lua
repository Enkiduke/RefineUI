...
function Module:OnEnable()
    ...
    if not db or not db.Installed or db.InstallState == "pending" then
        return
    end
...
function Module:ConfigureRefineUILayout()
    ...
    if LibEditModeOverride:GetActiveLayout() == "RefineUI" then
        ...
        if DebuffFrame then
			LibEditModeOverride:SetFrameSetting(DebuffFrame, Enum.EditModeAuraFrameSetting.IconWrap, Enum.AuraFrameIconWrap.Up)
			LibEditModeOverride:SetFrameSetting(DebuffFrame, Enum.EditModeAuraFrameSetting.IconDirection, Enum.AuraFrameIconDirection.Right)
            LibEditModeOverride:SetFrameSetting(DebuffFrame, Enum.EditModeAuraFrameSetting.IconSize, 5)
            LibEditModeOverride:SetFrameSetting(DebuffFrame, Enum.EditModeAuraFrameSetting.IconLimitDebuffFrame, 5)
            LibEditModeOverride:SetFrameSetting(DebuffFrame, Enum.EditModeAuraFrameSetting.IconPadding, 8)
        end
...
function Module:EnsureRefineUILayout(forceReload, allowCreate)
    ...
    local function try()
        ...
        -- Install/bootstrap state is only ready after layout apply succeeds.
        if allowCreate and db then
            db.Installed = true
            db.InstallState = "ready"
        end

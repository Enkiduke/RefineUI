...
function Install:DoInstall()
    ...
    -- 3. Enter install bootstrap state. Mark as ready only after EditMode apply succeeds.
    RefineUI.DB.Installed = false
    RefineUI.DB.InstallState = "pending"
    ...
    if EditMode and EditMode.EnsureRefineUILayout then
        ...
        RefineUI.DB.InstallState = "ready"
...
function Install:OnEvent(event)
    if event == "PLAYER_ENTERING_WORLD" then
        ...
        local isInstalled = db and db.Installed
        local installState = db and db.InstallState

        -- Pre-install/bootstrap path: do not touch EditMode APIs at all.
        if not isInstalled or installState == "pending" then
            if db and installState ~= "pending" then
                db.InstallState = "pending"
            end
        ...
        -- Installed path: verify layout still exists.
        -- Wait for EditMode Lib to be ready before querying layout data.

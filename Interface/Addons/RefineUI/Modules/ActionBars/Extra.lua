...
local function StyleExtraButton(button, isZone)
	...
	-- HotKey
	if hotkey then
		local showHotkey = Module.db and Module.db.ShowHotkeys and Module.db.ShowHotkeys["ExtraAction"]
		if showHotkey then
            ...
            if hotkey.SetShown then
                RefineUI:HookOnce(BuildExtraHookKey(hotkey, "SetShown"), hotkey, "SetShown", function(self, shown)
                    if shown then self:SetAlpha(0) end
                end)
            end
			RefineUI:HookOnce(BuildExtraHookKey(hotkey, "Show"), hotkey, "Show", function(self) self:SetAlpha(0) end)
		end
	end

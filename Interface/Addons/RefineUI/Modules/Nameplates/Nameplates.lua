...
local function ApplyConfiguredNameplateSize(unitFrame, _nameplate)
    ...
    -- Some Blizzard templates expose SetWidth but reject this object as self; guard with pcall.
...
local function EvaluateNameOnlyFromUnit(unit)
    if not IsUsableUnitToken(unit) then
        -- Match previous UpdateVisibility behavior: default to name-only when unit token is unavailable.
        return true
    end

    -- Keep this logic identical to UpdateVisibility's original behavior.
...
local function IsNameOnlyNameplateInternal(unitFrame, data, allowCachedState)
    if not unitFrame then
        return false
    end
    ...
    if type(unitFrame.IsShowOnlyName) == "function" then
        ...
        if ok and ReadSafeBoolean(isShowOnlyName) == true then
            return true
        end
    end

    if ReadSafeBoolean(unitFrame.showOnlyName) == true then
        return true
    end

    local optionTable = unitFrame.optionTable
    if ReadSafeBoolean(SafeTableIndex(optionTable, "nameOnly")) == true then
        return true
    end
    if ReadSafeBoolean(SafeTableIndex(optionTable, "showOnlyName")) == true then
        return true
    end
...
function RefineUI:IsNameOnlyNameplate(unitFrame, data, allowCachedState)
    return IsNameOnlyNameplateInternal(unitFrame, data, allowCachedState)
end

local function ApplyPortraitRaidIconAnchor(unitFrame, data)
...
local function ApplyNameOnlyRaidIconAnchor(unitFrame, data)
    if not unitFrame then
        return
    end
    ...
    -- Name-only plates: keep raid marker attached above the custom name text.
    raidTargetFrame:SetSize(RAID_ICON_SIZE, RAID_ICON_SIZE)
    raidTargetFrame:ClearAllPoints()
...
local function ApplyRaidIconAnchor(unitFrame, data, isNameOnlyOverride)
    if not unitFrame then
        return
    end
    ...
    if isNameOnly == nil then
        isNameOnly = RefineUI:IsNameOnlyNameplate(unitFrame, data)
    end

    if isNameOnly then
        ApplyNameOnlyRaidIconAnchor(unitFrame, data)
    elseif data then
        ApplyPortraitRaidIconAnchor(unitFrame, data)
    end
end

function RefineUI:UpdateNameplateRaidIconAnchor(unitFrame, data, isNameOnlyOverride)
    ApplyRaidIconAnchor(unitFrame, data, isNameOnlyOverride)
end
...
local function SkinNamePlateAura(frame)
    ...
    if GetNameplateState(frame, "AuraSkinned", false) then
...
local function UpdateVisibility(nameplate, unit)
    ...
    -- Use shared detector to keep visibility, target arrows, and raid icon anchoring in sync.
    -- Skip cached state here so this update can flip mode immediately when the unit state changes.
    local isNameOnly = IsNameOnlyNameplateInternal(unitFrame, data, false)

    local wasHidden = data.RefineHidden == true

    -- Shared name-only detection keeps visibility, arrows, and raid icon anchoring consistent.
    if isNameOnly then
        -- Friendly/Unattackable: Hide bars, show name only
        if healthContainer then
            healthContainer:SetAlpha(0)
        end
        ApplyRaidIconAnchor(unitFrame, data, true)
        ...
        ApplyRaidIconAnchor(unitFrame, data, false)
...
local function OnNameplateAdded(event, unit)
    ...
    if nameplate then
        ...
        UpdateHealth(nameplate, safeUnit)
...
function Nameplates:OnEnable()
    ...
    -- Keep raid marker anchored above the name text while the plate is in name-only mode.
    RefineUI:HookOnce("Nameplates:CompactUnitFrame_UpdateRaidTargetIcon", "CompactUnitFrame_UpdateRaidTargetIcon", function(frame)
        if frame:IsForbidden() then return end
        ...
        ApplyRaidIconAnchor(frame, data)
        ...
        if RefineUI.UpdateTarget then
            RefineUI:UpdateTarget(frame)
        end
    end)

    -- Blizzard re-runs UpdateAnchors/UpdateRaidTarget frequently; re-apply our raid icon anchor after those paths.
    if _G.NamePlateUnitFrameMixin then
        RefineUI:HookOnce("Nameplates:NamePlateUnitFrameMixin:UpdateRaidTarget:Anchor", _G.NamePlateUnitFrameMixin, "UpdateRaidTarget", function(frame)
        ...
        RefineUI:HookOnce("Nameplates:NamePlateUnitFrameMixin:UpdateAnchors:RaidAnchor", _G.NamePlateUnitFrameMixin, "UpdateAnchors", function(frame)
            if not frame or (frame.IsForbidden and frame:IsForbidden()) then return end

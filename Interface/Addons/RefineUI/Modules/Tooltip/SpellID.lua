...
function Tooltip:InitializeSpellID()
    ...
    if TOOLTIP_DATA_TYPE and TOOLTIP_DATA_TYPE.UnitAura then
        Tooltip:AddTooltipPostCallOnce(SPELL_ID_POSTCALL_UNIT_AURA_KEY, TOOLTIP_DATA_TYPE.UnitAura, function(tooltip, data)
            if not IsModifierKeyDown() then
                return
            end
            if tooltip ~= GameTooltip then

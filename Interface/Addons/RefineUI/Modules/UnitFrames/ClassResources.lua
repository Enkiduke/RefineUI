...
local function UpdateTotemBar(Resource)
    ...
    then
        for totemButton in totemFrame.totemPool:EnumerateActive() do
            if totemButton then
                ...
                if IsNonSecretNumber(layoutIndex) and layoutIndex >= 1 and layoutIndex <= MAX_TOTEMS then
    ...
    for i = 1, MAX_TOTEMS do
        ...
        local sourceButton = mappedByLayoutIndex[i]
        ...
        if isActive and Button then
            ...
            if sourceIcon and type(sourceIcon.GetTexture) == "function" then
                ...
                if okIcon and IsNonSecretTexture(resolvedTexture) then
            ...
            if sourceCooldown and type(sourceCooldown.GetCooldownTimes) == "function" then
                ...
                if okTimes
                    and not IsSecret(startMS)
                    and not IsSecret(durationMS)
            ...
            if not didApplyCooldown then
                ClearCooldownSafe(Button.Cooldown)
            end
...
function UF:CreateClassResources(Frame)
    ...
    local function ResolveResourceAnchor()
        ...
        if Frame.PlayerFrameContent and Frame.PlayerFrameContent.PlayerFrameContentMain then
            local hbContainer = Frame.PlayerFrameContent.PlayerFrameContentMain.HealthBarsContainer
            -- Use the container (stable width) instead of the health bar itself, because
            -- temporary max-health loss effects can shrink HealthBar and shift attached bars.
            if hbContainer then
                anchor = hbContainer
            end
        end
    ...
    local function CreateBaseBar(name, type, width, height, yOff)
        ...
        if existing then
            ...
            if pos then
                existing:Point(pos[1], anchor, pos[3], pos[4], pos[5])
            else
                existing:Point("BOTTOM", anchor, "TOP", 0, yOff or DB.YOffset or 4)
            end

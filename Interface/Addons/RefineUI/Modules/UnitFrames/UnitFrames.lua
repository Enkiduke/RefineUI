...
local function IsTargetFocusOrBossFrame(frame)
    if not frame then
        return false
    end
...
local function ApplyRaidTargetIconAnchor(frame, contentContext)
    if not IsTargetFocusOrBossFrame(frame) or not contentContext then
        return
    end
    ...
    if not raidTargetIcon.RefineRaidIconAnchorHooked then
        RefineUI:HookOnce(BuildUnitFrameHookKey(raidTargetIcon, "SetPoint:RaidTargetAnchor"), raidTargetIcon, "SetPoint", function(self)
            if self.RefineApplyingRaidIconAnchor then
                return
            end
            self.RefineApplyingRaidIconAnchor = true
            self:ClearAllPoints()
            self:SetPoint("RIGHT", frame, "LEFT", 0, 0)
            self.RefineApplyingRaidIconAnchor = false
        end)
        raidTargetIcon.RefineRaidIconAnchorHooked = true
    end

    raidTargetIcon.RefineApplyingRaidIconAnchor = true
    raidTargetIcon:ClearAllPoints()
    raidTargetIcon:SetPoint("RIGHT", frame, "LEFT", 0, 0)
    raidTargetIcon.RefineApplyingRaidIconAnchor = false
end
...
StyleUnitFrame = function(frame)
    ...
    -- 2c. Hide Attack/Quest Icons (Player/Target/Focus)
    if contentContext then
        EnforceHiddenRegion(contentContext.AttackIcon, hiddenFrame)
        if frame == TargetFrame or frame == FocusFrame then
            EnforceHiddenRegion(contentContext.QuestIcon, hiddenFrame)
        end
    end

    -- 2d. Hide Prestige Badge / Portrait (PvP Honor Icon)
    if contentContext then
        EnforceHiddenRegion(contentContext.PrestigeBadge, hiddenFrame)
        EnforceHiddenRegion(contentContext.PrestigePortrait, hiddenFrame)

        if frame == TargetFrame or frame == FocusFrame then
            EnforceHiddenRegion(contentContext.HighLevelTexture, hiddenFrame)
        end
    end

    -- 2e. Disable Tooltips (Shift Overlay)
    ...
    if level then
        if frame == PlayerFrame then
            ...
            EnforceHiddenRegion(level, hiddenFrame)
        end

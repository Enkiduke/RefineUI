...
function oGlow:RegisterFilterOnPipe(pipe, filter)
    if not pipe or not filter then
        print("Error: RegisterFilterOnPipe called with invalid arguments. Pipe:", pipe, "Filter:", filter)
        return nil, "Invalid arguments."
    end

    if not pipesTable[pipe] then
        print("Error: Pipe does not exist:", pipe)
        return nil, "Pipe does not exist."
    end

    if not filtersTable[filter] then
        print("Error: Filter does not exist:", filter)
    ...
    if not activeFilters[pipe] then
        ...
        if not ref then
            ref = {}
            activeFilters[pipe][display] = ref
        end

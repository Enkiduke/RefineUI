...
local RECONCILE_TIMER_KEY = "CooldownAuraTrackers:AuraProbe:Reconcile"
...
local function ResolveIconFromCooldownID(cooldownID)
    ...
    local info = Module:GetCooldownInfo(cooldownID)
    if type(info) ~= "table" then
        return nil
    end

    local texture = ResolveIconFromSpellID(Module:ResolveCooldownSpellID(info))
    if texture then
...
local function InstallFrameHooks(frame)
    ...
    if type(frame.SetCooldownID) == "function" then
        RefineUI:HookOnce("CooldownAuraTrackers:AuraProbe:" .. hookID .. ":SetCooldownID", frame, "SetCooldownID", function(hookedFrame, cooldownID)
    ...
    if type(frame.ClearCooldownID) == "function" then
        RefineUI:HookOnce("CooldownAuraTrackers:AuraProbe:" .. hookID .. ":ClearCooldownID", frame, "ClearCooldownID", function(hookedFrame)
            UnregisterCooldownFrame(hookedFrame)
            Module:RequestRefresh()
        end)
    end
...
local function InstallViewerHooks(viewer, viewerName)
    ...
    if type(viewer.OnAcquireItemFrame) == "function" then
        RefineUI:HookOnce("CooldownAuraTrackers:AuraProbe:" .. hookID .. ":OnAcquireItemFrame", viewer, "OnAcquireItemFrame", OnViewerReshuffle)
    end
    if type(viewer.RefreshData) == "function" then
        RefineUI:HookOnce("CooldownAuraTrackers:AuraProbe:" .. hookID .. ":RefreshData", viewer, "RefreshData", OnViewerDataRefresh)
    end
    if type(viewer.RefreshLayout) == "function" then
        RefineUI:HookOnce("CooldownAuraTrackers:AuraProbe:" .. hookID .. ":RefreshLayout", viewer, "RefreshLayout", OnViewerReshuffle)
    end
...
local function StoreGhostPayload(cooldownID, payload)
    local ttl = Module.GetPayloadGhostTTL and Module:GetPayloadGhostTTL() or 0.20
...
local function ShouldReplacePayload(existing, candidate)
    ...
    -- Prefer the latest observed payload when timing is secret/non-comparable.
    return true
end
...
local function TryRegisterDataChangedCallback()
    ...
    eventRegistry:RegisterCallback("CooldownViewerSettings.OnDataChanged", function()
        local settingsFrame = Module.GetCooldownViewerSettingsFrame and Module:GetCooldownViewerSettingsFrame()
        if settingsFrame and settingsFrame:IsShown() and Module.MarkReloadRecommendationPending then
            Module:MarkReloadRecommendationPending()
        end
        if Module.MarkAssignmentsPruneDirty then
            Module:MarkAssignmentsPruneDirty()
        end
        MarkRegistryDirty()
        QueueRegistryReconcile()
    end, Module)
...
function Module:InvalidateAuraProbeCache()
    MarkRegistryDirty()
    if wipe then
...
function Module:RequestAuraProbeReconcile()
    MarkRegistryDirty()
    QueueRegistryReconcile()
end

function Module:InitializeAuraProbe()
    ...
    RefineUI:RegisterEventCallback("ADDON_LOADED", function(_event, addonName)
        if addonName == "Blizzard_CooldownViewer" then
            InstallKnownViewerHooks()
            TryRegisterDataChangedCallback()
            Module:InvalidateAuraProbeCache()
            Module:RequestAuraProbeReconcile()
        end
    end, "CooldownAuraTrackers:AuraProbe:AddonLoaded")

    self.auraProbeInitialized = true
end

function Module:ProbeCooldownAura(cooldownID, activeFrameMap)
...
function Module:GetActiveAuraMap(cooldownIDs)
    local activeMap = {}
